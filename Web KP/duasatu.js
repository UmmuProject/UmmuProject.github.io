let dataStorage = JSON.parse(localStorage.getItem('duasatuDataStorage')) || [];
...
function handleFormSubmit() {
    ...
    reader.onload = function(event) {
        ...
        // Filter data to include only relevant columns and rows with specified Plant codes
        const filteredData = json
            .filter((row, index) => {
                if (index === 0) return true; // Keep header row
                const plantCode = row[headers.indexOf('Plant')];
                return ['F239', 'F3BG', 'F3BT'].includes(plantCode);
            })
...
function saveData() {
    localStorage.setItem('duasatuDataStorage', JSON.stringify(dataStorage));
